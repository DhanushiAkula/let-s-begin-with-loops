...
while serving:
    ...
    if amount<=0:
        print("invalid amount.please enter the positive number.\n")
        continue
...
for slot in range(1,7):
    ...
    if total>0:
        print(f"{value}-unit notes dispensend:{total}",end="")
        for note in range(total):
            print("=",end="")
        print()
print(f"\nCustomers served:{customers_served}")
print(f"total dispensend:{total_dispensed}units")
print("ATM session closed.gooodbye!")
